...
const hookRespond = require('./../responders/hookrespond');
...
function execute(command, params, callback) {
  ...
  if (alias.isAlias(commandName)) {
    const commands = parser.getCommands(alias.get(commandName));
    ...
    const data = helpers.packageData(event, params, hook.parent, commandArgs);
    const responder = hookRespond.assignResponder(commandName, data);
...
function fireTriggers(event, params, commandOnly) {
  ...
  async.each(triggers[event], function(hook) {
    ...
    if (hook.trigger(params)) {
      ...
      const responder = hookRespond.assignResponder(hook.name, data);
      hook.action(data, responder, function() { responder.flush(); });
    }
  }, function(err) {});
}
...
module.exports = {
  ...
  fire(event, params, callback) {
    if (event === 'message' && parser.isCommand(params.text)) {
