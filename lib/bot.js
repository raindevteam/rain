...
class Bot extends irc.Client {
  ...
  /*
   * Runs after connecting to IRC, identifies with NickServ and sends
   * any modes defined in the config. Joins channels listed in config lastly.
   */

  postStart() {
    rainlog.info('Bot', 'Bot connected to IRC');
    if (nsPassword) self.send('ns', 'identify', nsPassword);
    if (__config.modeList) self.send('mode', __config.nick, __config.modeList);
